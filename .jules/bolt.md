...
## 2025-02-12 - Batch list appends per directory
**Learning:** In highly nested loops like file tree traversal with `os.walk`, making numerous individual `.append()` calls to a main list results in high method call overhead in the innermost hot path. While list comprehensions and generators seem idiomatic, they sometimes run slower due to extra generator or nested function call overhead if extracting the body into a separate function.
**Action:** The fastest pure-Python optimization for this pattern without structural changes is to 1) hoist the `.extend()` method of the main list outside the loop, 2) inside the top-level directory iteration, create a temporary array and hoist its `.append()` method, and 3) use the hoisted `.append()` in the inner loop. Finally, batch extend the main list if the temporary array has items.
