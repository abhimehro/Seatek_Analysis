...
**Prevention:** To secure `openpyxl` against XML-based attacks, the `defusedxml` package must be installed in the environment. `openpyxl` automatically detects and uses `defusedxml` for parsing if it's available, completely neutralizing these attack vectors. Always explicitly list `defusedxml` alongside `openpyxl` in `requirements.txt`.
