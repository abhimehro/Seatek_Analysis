...
test_that("dump_summary_excel creates correct Excel and CSV files", {
  ...
  # testthat runs tests with working directory set to the test file dir.
  # So getwd() inside test-dump_summary_excel.R will be tests/testthat
  ...
  # Generates a data.table for one year with some offset to make data unique
  ...
  # Create a list of such data.frames, named by year (5 years)
  ...
  expect_true(
    file.exists(output_excel_file), label = "Excel file must be created."
  )
  ...
  # Also check if "Summary_Top_Sensors" is present as dump_summary_excel adds it
  expect_true(
    "Summary_Top_Sensors" %in% sheet_names,
    label = "Summary_Top_Sensors sheet must be present."
  )


  # 3. Check CSV file creation
  csv_base_name <- tools::file_path_sans_ext(output_excel_file)
  ...
  csv_files_to_check <- c(
    ...
    # Corresponds to "Summary_Robust" sheet (if created)
  ...
  for (csv_file_path in csv_files_to_check) {
    ...
    if (grepl("_sufficient.csv", csv_file_path, fixed = TRUE)) {
      ...
      expect_gt(
        nrow(df_sufficient_csv), 0,
        label = "The _sufficient.csv should not be empty."
      )
      # Given 5 years of mock data, all 32 sensors should be present.
      expect_equal(
        nrow(df_sufficient_csv), 32,
        label = "The _sufficient.csv should contain 32 sensor rows."
      )
  ...
  value_from_excel <- data_from_1995_sheet[
    data_from_1995_sheet$Sensor == "Sensor01", "first10"
  ]
  ...
  value_from_excel_s32_full <- data_from_1999_sheet[
    data_from_1999_sheet$Sensor == "Sensor32", "full"
  ]
  ...
  unlink(temp_dir_path, recursive = TRUE, force = TRUE)
  expect_false(
    dir.exists(temp_dir_path),
    label = "Temporary directory should be deleted after test."
  )
})
