...
## 2025-05-06 - R Method Dispatch Overhead in Grouped Operations
**Learning:** Using S3 generic functions like `tail()` inside highly iterative or grouped operations (like `lapply(.SD)` in `data.table`) introduces significant method dispatch overhead. For simple data types like numeric vectors, native operations like vector indexing (e.g., `v_val[(n-2):n]`) are drastically faster. Additionally, using mathematical equivalents like `sum() / 3` instead of `mean()` for small fixed-length vectors avoids further generic dispatch overhead.
**Action:** When writing performance-critical R code, especially within `data.table` grouping or tight loops, replace high-level generic functions (like `tail()`, `head()`, `mean()`) with direct indexing and primitive arithmetic operations where the data type and bounds are known and safe.
