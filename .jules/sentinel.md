...
## 2026-06-01 - TOCTOU Vulnerability with File Buffers
**Vulnerability:** A script correctly checks the file size of a file before reading it into an in-memory buffer (`file_buffer = f.read()`) to prevent OOM DoS, but a downstream function takes the original file path instead of the `file_buffer`, reopening the file. This introduces a Time-Of-Check to Time-Of-Use (TOCTOU) vulnerability where an attacker can swap the file with a malicious/larger one between the check and the actual parsing.
**Learning:** Checking a file, reading it into a buffer, and then separately passing the original file path down to other functions creates a window for TOCTOU attacks if those downstream functions re-read from disk.
**Prevention:** Once a file is securely read into memory (after validation), all subsequent functions should accept and process the memory buffer (e.g. `BytesIO(file_buffer)`) rather than re-reading the file path from disk.
## 2025-07-11 - Credential Exfiltration via Subprocess Environment
**Vulnerability:** The `run_process` and `run_shell_command` functions in `.github/scripts/repository_automation_common.py` strip `GH_TOKEN` from the environment to prevent credential exfiltration. However, the `get_repo_info` function in `code_health_scanner.py` uses `subprocess.check_output` to run `git` commands but does not strictly limit the environment passed to it, only attempting to `env.pop("GH_TOKEN", None)`. While this specific script may only run `git`, any external command invoked via `subprocess` inherits the environment by default, potentially exposing other sensitive variables if the subprocess is compromised or if it's a third-party tool.
**Learning:** Explicitly stripping a single token like `GH_TOKEN` is a denylist approach and is fragile. Other tokens might be added later and leaked.
**Prevention:** When running `subprocess` commands, construct a minimal, explicit `env` dictionary containing only the variables the command strictly needs, rather than copying and modifying the entire `os.environ` (allowlist approach).

## 2025-02-14 - Enhanced Subprocess Environment Denylisting
**Vulnerability:** Subprocess execution wrappers (`run_process`, `run_shell_command`) used an incomplete denylist (`env.pop("GH_TOKEN")`) to sanitize the environment before executing external commands, leaving other identical tokens like `GITHUB_TOKEN` vulnerable.
**Learning:** While a strict allowlist approach for environments is the most secure, applying it broadly in CI/CD automation can break essential tools that rely on system variables (like `PATH`, `HOME`, `USER`) or CI-injected context (`GITHUB_WORKSPACE`, `GITHUB_ENV`).
**Prevention:** When an allowlist is functionally impossible, ensure the denylist comprehensively covers all known sensitive credentials exposed to the CI environment (e.g., stripping both `GH_TOKEN` and `GITHUB_TOKEN`), and continuously audit for new tokens added to the runtime environment.
