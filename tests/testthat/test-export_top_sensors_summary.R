...
test_that("export_top_sensors_summary works correctly", {
  ...
  mock_summary_df <- data.table(
    ...
    # Absolute values: 10, 9, 8, 7, 6, 5, 4, 3
    within_diff_mean = c(10, -9, 8, -7, 6, -5, 4, -3),
  ...
  expect_true(
    "Summary_Top_Sensors" %in% names(wb),
    label = "Summary_Top_Sensors sheet should be added to the workbook."
  )
  ...
  expect_equal(
    nrow(top_csv), 5,
    label = "There should be exactly 5 rows in the top sensors summary."
  )

  # Check columns
  expected_cols <- c(
    "Sensor", "within_diff_mean", "full_mean", "full_sd", "full_pct_nonmissing"
  )
  expect_equal(
    colnames(top_csv), expected_cols,
    label = "The columns in top sensors summary should match expected columns."
  )
  ...
  expect_equal(
    top_csv$Sensor, expected_sensors,
    label = "Sensors should be ordered by absolute within_diff_mean."
  )
  ...
  suppressMessages({
    export_top_sensors_summary(
      wb2, mock_summary_no_diff, output_file2, header_style
    )
  })

  # Sheet should not be added and CSV not created
  expect_false(
    "Summary_Top_Sensors" %in% names(wb2),
    label = "Sheet should not be added if within_diff_mean is missing."
  )
  expect_false(
    file.exists(csv_file2),
    label = "CSV should not be created if within_diff_mean is missing."
  )
