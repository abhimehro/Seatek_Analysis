...
# Use the global variables if needed (assuming testthat.R setup loads them).
...
test_that("write_year_sheet works correctly", {
  ...
  # Verify dimensions
  expect_equal(
    nrow(read_data), nrow(mock_data), label = "Number of rows should match."
  )
  expect_equal(
  ...
  expect_equal(
    read_data$Sensor, mock_data$Sensor, label = "Sensor column should match."
  )
  expect_equal(
  ...
  expect_true(
    found_highlight,
    label = "Highlight style applied to correct cell (row 3, col 5)."
  )
  ...
  # 4. Test without 'within_diff' column (should not fail or highlight)
  wb3 <- createWorkbook()
